...
const apiKey = 'b512158c4d2f5488c21ecef3a8f5f72a';
  ...
  fetch(trends)
    ...
    .then(function (output) {
      ...
      output.results.forEach((item) => {
        ...
        let nameItem = item.name || item.title,
          img = item.poster_path === null ? `<img src="./img/noposter.jpeg" class="card-img-top img-fluid img-thumbnail"alt="no poster" ${dataInfo}>` : `<img src="${imgHost}${item.poster_path}"class="card-img-top img-fluid img-thumbnail" alt="${nameItem}"  ${dataInfo}>`,
          overview = item.overview,
          itemDate = (item.release_date !== "" && item.release_date !== undefined) ? (new
            Date(Date.parse(item.release_date))).toLocaleString("ru", {
              day: 'numeric', month: 'long', year: 'numeric',
